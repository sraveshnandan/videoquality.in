...
const TaskPage = () => {
  ...
  const hgandleTaskCreate = async (ev: FormEvent<HTMLFormElement>) => {
    ...
    formdata.append("video", rawvideo as any);
    setloading(true);
    ...
    if (!res.success) {
      ...
      return toast.error("Unable to upload the video.");
    }

    // creating the task

    const payload = {
      rawVid: res.video_url,
      prompt,
      editing_type,
    };
  ...
  return loading ? (
    ...
    <section className="flex flex-col   h-full  rounded-md overflow-hidden lg:items-start items-center">
      ...
      {profile && !taskstae && profile.tasks.length > 0 ? (
        <>
          <span className="text-white text-4xl font-semibold lg:text-start text-center mb-4">
            Your previous tasks
          </span>
            ...
            {profile.tasks.map((item: any, index: number) => (
              ...
              >
                ...
                <div className="flex flex-col w-1/3">
                  <span className="text-white line-clamp-1">Task Id</span>
                ...
                {/* prompt section  */}
                <div className="flex flex-col w-1/3 ">
                  <span className="text-white line-clamp-1">Created At:</span>
                  <span className="text-white line-clamp-1">
                ...
                <div className="flex flex-col w-1/3">
                  ...
                  <span className="text-white line-clamp-1">{item.status}</span>
                </div>

                {/* View more section  */}

                <div className="flex flex-col items-center justify-center">
                  <span className="text-white line-clamp-1">Details</span>
        ...
        <>
          ...
          {taskstae ? (
            <section className="flex w-full h-full flex-col   items-center justify-center">
              <Uploader rawVideoFile={setrawvideo} />

              {/* prompt  */}

              <form
                onSubmit={hgandleTaskCreate}
                className="flex flex-col text-white bg-glass my-4 lg:w-[50%] w-full rounded-md  p-2"
              >
                {/* video editing type  */}
                <label
                  htmlFor="ed_type"
            ...
            <section className="flex w-full h-full flex-col  border items-center justify-center">
              ...
              <span
                onClick={() => settaskstae(true)}
                className="hover:text-white duration-300 transition-all px-5 py-2 rounded-md my-4 hover:bg-gradient-to-tr from-pink-500 to-indigo-500 hover:shadow-lg hover:shadow-pink-500 cursor-pointer bg-white"
              >
                Add task
              </span>
            </section>
